...
- 脚注（注釈）は `(())` 形式で記載する
  - 例：`本文中に注を入れたい場合((このように注釈を記載します))`
  - 複数行にまたがる注の場合も `(())` で囲む
  - 引用元を明記する際に使用する
- 一つの説明に複数の脚注を紐づける場合は、HTMLタグを使用して対応づけることができる
  - 例：`説明文<sup id="fnref:1"><a href="#fn:1" rel="footnote">1</a></sup><sup id="fnref:2"><a href="#fn:2" rel="footnote">2</a></sup>`
  - 複数の出典がある場合に有効
